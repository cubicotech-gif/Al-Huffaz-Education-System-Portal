...
class Sponsor_Dashboard {
    ...
    public static function get_student_progress($student_id, $user_id) {
        ...
        return array(
            ...
            'category'          => Helpers::get_islamic_category_label(get_post_meta($student_id, '_islamic_category', true)),
            'overall_percentage'=> get_post_meta($student_id, '_overall_percentage', true),
            'overall_grade'     => Helpers::get_grade_from_percentage(get_post_meta($student_id, '_overall_percentage', true)),
            'attendance'        => \AlHuffaz\Admin\Student_Manager::get_attendance($student_id),
            'subjects'          => get_post_meta($student_id, '_subjects', true) ?: array(),
            'teacher_comments'  => get_post_meta($student_id, '_teacher_comments', true),
            'goals'             => get_post_meta($student_id, '_goals', true) ?: array(),
        );
    }
}
